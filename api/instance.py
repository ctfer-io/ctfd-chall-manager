...
class UserInstance(Resource):
    ...
    def post():  # pylint: disable=too-many-return-statements,too-many-branches
        ...
        user_id = int(user.id)
        source_id = user_id
        user_email = str(user.email)  # Custom Invictus: get user email
        logger.info(
        ...
        try:
            ...
            # Custom Invictus: pass user_email to challenge manager
            result = create_instance(challenge_id, source_id, user_email)
            logger.info(
